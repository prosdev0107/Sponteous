...
module.exports = {
  ...
  hasOpposite(trip, allTrip) {
    const oppositeTrip = undefined;
    ...
    // return null
    for (const otrip of allTrip) {
      if (trip.departure.name == otrip.destination && trip.destination.name == otrip.departure && trip.carrier == otrip.carrier && trip.type == otrip.type) {
  ...
  async hasEnoughTickets(trip, allTicket, allTrip) {
    const oppositeTrip = this.hasOpposite(trip, allTrip)
    let oppositeTickets = []
    let tripTickets = []
    ...
    for (const ticket of trip.tickets) {
      ...
      for (const aticket of allTicket) {
        if (aticket._id == ticket._id.toString())
          tripTickets.push();
        return;
      }
    }
  ...
  async findDashboard({ page, limit, adult, youth, priceStart, priceEnd, dateStart, dateEnd, departure, timezone }) {
    ...
    for (const trip of data) {
      if (this.hasEnoughTickets(trip, allTicket,allTrip)) {
        trip["Adult"] = adult
        trip["Youth"] = youth
